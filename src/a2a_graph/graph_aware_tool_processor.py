...
class GraphAwareToolProcessor:
    ...
    def __init__(
        self,
        session_id: str,
        graph_store: Any,  # Interface for storing/retrieving graph nodes/edges
        *,
    ...
    async def process_llm_message(
        ...
        # 1. Create a run to track this execution
        run = session.create_run()
        run.mark_running()
        store.save(session)
...
# ------------------------------------------------------------------------
# Example Graph Store implementation (for demonstration)
# ------------------------------------------------------------------------

class SimpleGraphStore:
    """
    Simple in-memory graph store for demonstration purposes.
