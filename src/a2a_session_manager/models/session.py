...
class Session(AccessControlled, BaseModel, Generic[MessageT]):
    ...
    def _sync_hierarchy(cls, model: Session) -> Session:
        """
        After model creation, if this session has a parent_id,
        fetch that parent from the store and append this session’s ID
        to its child_ids (if not already present).
        """
        if model.parent_id:
    ...
    def ancestors(self) -> List[Session]:
        """Return the chain of ancestor sessions, nearest first."""
        result: List[Session] = []
    ...
    def descendants(self) -> List[Session]:
        """Return all descendant sessions in depth-first order."""
        result: List[Session] = []
