...
from a2a_session_manager.infinite_conversation import InfiniteConversationManager
...
async def test_error_handling_in_llm_callback(store):
    ...
    with pytest.raises(RuntimeError):
        await mgr.process_message(sess.id, "x", EventSource.USER, bad)

    # Even though callback failed, message should have been recorded
    upd = await store.get(sess.id)
